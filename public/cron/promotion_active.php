...
//    $date = new \DateTime();
    $sth = $dbh->prepare( "SELECT * FROM promotion WHERE debut='".date('Y-m-d')."'");
    $sth->execute();
...
//            $s = $dbh->prepare( "UPDATE produit SET promotion_id =".$promotion['id']." WHERE id=".$promotionproduit['produit_id']);
//            $s->execute();
